...
contract OneInchTest is Test {
    ...
    // ############################
    string url = "https://api.1inch.dev/swap/v6.0/8453/swap";
    string rpcUrl = "BASE_RPC";
    string path = "base_data.csv";
    // ############################
    ...
    function testSimulate1Inch() public {
        ...
        for (uint256 i = 0; i < outTokens.length; i++) {
            ...
            for (uint256 j = 0; j < 100; j++) {
                ...
                if (!successModified) {
                    console.log("Failed for token ", outToken, " with amount ", amountInWithSlippage);
                    continue;
                } else {
    ...
    function _get1InchCalldata(address outToken, uint256 randAmountIn) internal returns (bytes memory, uint256) {
        ...
        require(status == 200, "Failed to fetch 1inch data");
